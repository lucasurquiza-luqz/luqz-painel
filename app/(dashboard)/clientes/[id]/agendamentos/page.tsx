...
export default async function ClienteAgendamentosPage({ params }: { params: Promise<{ id: string }> }) {
              ...
              <div key={msg.id} className="bg-zinc-900 border border-white/8 rounded-2xl px-5 py-4 flex items-start gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-2">
                    <span className="text-xs text-zinc-500">{groupNames || "Sem grupos"}</span>
                    <span className="text-xs text-zinc-500">
                      {formatInTimeZone(msg.scheduledAt, TZ, "dd/MM/yyyy 'às' HH:mm", { locale: ptBR })}
                    </span>
                    {msg.mediaName && <span className="text-xs text-zinc-600">Arquivo: {msg.mediaName}</span>}
