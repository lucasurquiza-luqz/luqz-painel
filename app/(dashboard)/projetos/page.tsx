...
export default function ProjetosPage() {
  ...
  return (
    <main className="mx-auto max-w-5xl space-y-5 p-6 lg:p-8">
      ...
      {creating && <CreateProject onClose={() => setCreating(false)} onCreated={() => { setCreating(false); void load() }} />}
    </main>
  )
}
...
function CreateProject({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  async function submit() {
    ...
    setBusy(true); setErr("")
    const res = await fetch("/api/projects", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(f) })
    setBusy(false)
