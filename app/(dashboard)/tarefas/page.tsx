...
export default function TarefasPage() {
  ...
  return (
    <main className="mx-auto max-w-6xl space-y-5 p-6 lg:p-8">
      ...
      {creating && <CreateModal team={team} projects={projects} onClose={() => setCreating(false)} onCreated={(t) => { setTasks((ts) => [t, ...ts]); setCreating(false) }} />}
...
function CreateModal({ team, projects, onClose, onCreated }: { team: Ref[]; projects: Ref[]; onClose: () => void; onCreated: (t: Task) => void }) {
  const [f, setF] = useState({ title: "", description: "", assigneeId: "", projectId: "", priority: "MEDIA", dueDate: "" })
