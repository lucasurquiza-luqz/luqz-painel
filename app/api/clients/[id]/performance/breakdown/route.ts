...
import { fetchMetaBreakdown } from "@/lib/ads/meta"
import { effectiveObjectives, type AdConfig, type AdObjective, type BreakdownLevel } from "@/lib/ads/types"

type Params = { params: Promise<{ id: string }> }
const LEVELS = new Set(["campaign", "adset", "ad"])

// Melhores campanhas / públicos (adset) / criativos (ad) do Meta no mês.
export async function GET(req: NextRequest, { params }: Params) {
  ...
  if (!level || !LEVELS.has(level)) return NextResponse.json({ error: "Nível inválido." }, { status: 400 })
  ...
  try {
    const rows = await fetchMetaBreakdown(account.accountId, decryptSecret(account.tokenEnc), month, config, level as BreakdownLevel)
    return NextResponse.json({ rows })
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Falha ao ler breakdown." }, { status: 502 })
  }
}
